        ...
        <Card className={glassCard}>
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
            <div className="flex-1 flex items-center gap-3 bg-white/[0.02] border border-white/[0.08] rounded-xl px-4 py-2 focus-within:ring-1 focus-within:ring-[#fbbf24]/50 transition-all">
              ...
              <Input placeholder="Search by ID, name, or email..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="bg-transparent border-0 text-white text-sm focus-visible:ring-0 shadow-none px-0 placeholder:text-zinc-600" />
            </div>
            <Select value={filterStatus} onValueChange={setFilterStatus}>
              <SelectTrigger className="w-full md:w-48 h-12 text-sm bg-white/[0.02] border-white/[0.08] text-white rounded-xl">
            ...
            <Button variant="outline" className="h-12 bg-white/[0.03] border-white/[0.1] text-white hover:bg-white/[0.1] rounded-xl px-6 font-bold">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-white/[0.08] bg-white/[0.02]">
                    <th className="py-4 px-6 text-left text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest">Transaction ID</th>
                    ...
                    <th className="py-4 px-6 text-right text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest">Amount</th>
                    <th className="py-4 px-6 text-left text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest">Source</th>
                    <th className="py-4 px-6 text-left text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest">Date</th>
                    <th className="py-4 px-6 text-left text-[10px] font-extrabold text-zinc-500 uppercase tracking-widest">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredTransactions.map((txn) => (
                    <tr key={txn.id} className="border-b border-white/[0.05] hover:bg-white/[0.02] transition-colors">
                      <td className="py-4 px-6 font-mono text-zinc-400">{txn.id.substring(0, 8)}...</td>
                      <td className="py-4 px-6 text-white font-bold">{txn.user}</td>
                      <td className="py-4 px-6 text-zinc-400">{txn.email}</td>
                      <td className="py-4 px-6 text-zinc-300 font-medium">{txn.plan}</td>
                      <td className="py-4 px-6 text-right text-[#fbbf24] font-black">${Number(txn.amount).toFixed(2)}</td>
                      ...
                      <td className="py-4 px-6 text-zinc-400 font-medium">{new Date(txn.date).toLocaleDateString()}</td>
                      <td className="py-4 px-6">
                        <span className={`text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-md border ${
